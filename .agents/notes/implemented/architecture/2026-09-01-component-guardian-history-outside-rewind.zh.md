...
guardian 从事件链派生已知快照与候选尝试索引。它对总事件数和单个候选的尝试次数施加有限上限，而 [`rehydrate`](../../../../packages/mykrobial/component-lifecycle/src/index.ts) 会重放每个事件并重建这些索引，然后才接受序列化快照。激活、回退或重启都不会删除先前的 guardian 事件。
