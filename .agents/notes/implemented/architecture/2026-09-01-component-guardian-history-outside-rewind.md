...
The guardian derives known snapshot and candidate-attempt indexes from the event chain. It applies finite limits to total events and attempts for one candidate, and [`rehydrate`](../../../../packages/mykrobial/component-lifecycle/src/index.ts) reconstructs those indexes by replaying every event before accepting a serialized snapshot. An activation, rewind, or restart never removes prior guardian events.
