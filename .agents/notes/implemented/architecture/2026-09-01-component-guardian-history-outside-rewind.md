...
The guardian derives known snapshot, candidate-attempt, and exact candidate-and-proposal association indexes from the event chain. An activation must name a proposal previously recorded for that candidate. The durable input path rejects boxed scalar aliases, sparse or accessor-backed arrays, custom properties, symbol keys, non-intrinsic array prototypes, subclasses, and Proxy aliases before hashing. The guardian applies finite limits to total events and attempts for one candidate, and [`rehydrate`](../../../../packages/mykrobial/component-lifecycle/src/index.ts) reconstructs those indexes by replaying every event before accepting a serialized snapshot. An activation, rewind, or restart never removes prior guardian events.
